...
class PyramidVisionTransformerV2(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        print(f"[PVTv2 init] Initial img_size: {img_size}, type: {type(img_size)}")
        for i in range(num_stages):
            current_img_size_for_patch_embed = img_size if i == 0 else img_size // (2 ** (i + 1))
            print(f"[PVTv2 loop {i}] img_size for OverlapPatchEmbed: {current_img_size_for_patch_embed}") # 추가
            if current_img_size_for_patch_embed is None: # 명시적 None 체크
                raise ValueError(f"img_size became None at stage {i}")
    ...
    # 공통 부분들 / PVTv2 정의 및 초기화까지 이후 pretrained load 혹은 decoder 추가 가능
                
    # 가중치 초기화 부분 (pretrained가 있을 경우)
    ...
    def forward_features(self, x):
        ...
        return outs # classification의 경우 feature map을 1-dimension으로 pooling해야하므로 이후 추가 처리


    def forward(self, x):
        x = self.forward_features(x)
        # x = self.head(x) / classification의 head이므로 주석처리
